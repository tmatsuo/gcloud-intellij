...
public class AppEngineSupportProvider extends FrameworkSupportInModuleProvider {
  ...
  private void addSupport(final Module module,
    ...
    if (persistenceApi != null) {
      ...
      facetConfiguration.getFilesToEnhance().addAll(AppEngineUtilLegacy.getDefaultSourceRootsToEnhance(rootModel));
      try {
        ...
        if (persistenceApi == PersistenceApi.JDO || persistenceApi == PersistenceApi.JDO3) {
          createFileFromTemplate(AppEngineTemplateGroupDescriptorFactory.APP_ENGINE_JDO_CONFIG_TEMPLATE, metaInf, AppEngineUtilLegacy.JDO_CONFIG_XML_NAME);
        }
        else {
          final VirtualFile file = createFileFromTemplate(AppEngineTemplateGroupDescriptorFactory.APP_ENGINE_JPA_CONFIG_TEMPLATE, metaInf, AppEngineUtilLegacy.JPA_CONFIG_XML_NAME);
          if (file != null) {
