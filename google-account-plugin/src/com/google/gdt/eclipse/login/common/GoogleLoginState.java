...
public class GoogleLoginState {
   ...
   * @param transport an instance of {@link HttpTransport}
   * @param jsonFactory an instance of {@link JsonFactory}
   */
...
//    }
    ...
    VerificationCodeHolder verificationCodeHolder =
        uiFacade.obtainVerificationCodeFromExternalUserInteraction(title, oAuthScopes);
  ...
  private void retrieveSavedCredentials() {
    ...
    com.google.gdt.eclipse.login.common.OAuthData savedAuthState = authDataStore.loadOAuthData();
    ...
    oAuthScopes.addAll(savedAuthState.getStoredScopes());
...
//    }
  ...
  private void saveCredentials() {
    if (!isLoggedIn) {
      ...
      OAuthData creds = new OAuthData(accessToken, refreshToken, email, oAuthScopes, accessTokenExpiryTime);
      authDataStore.saveOAuthData(creds);
    }
  }

  public SortedSet<String> getOAuthScopes() {
    return oAuthScopes;
  }
}
